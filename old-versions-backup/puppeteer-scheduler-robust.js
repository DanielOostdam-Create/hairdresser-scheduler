...
class PuppeteerCalendlyScheduler {
    ...
    // Helper method for delays (alternative to waitForTimeout/waitForDelay)
    async delay(ms) {
    ...
    async scrapeCalendlyWithPuppeteer(monthOffset = 0) {
        ...
        try {
            ...
            // Additional wait to ensure all dynamic content loads - FIXED VERSION
            console.log('⏳ Waiting for dynamic content...');
            try {
                // Try the new method first
                await page.waitForDelay(3000);
            } catch (error) {
                // Fallback to custom delay if waitForDelay doesn't exist
                console.log('⚠️ Using fallback delay method...');
...
async function main() {
    ...
    switch (command) {
        ...
        default:
            ...
            console.log('  node puppeteer-scheduler.js install     # Install Puppeteer');
            console.log('');
            console.log('Usage:');
            console.log('  node puppeteer-scheduler.js start       # Start monitoring');
            console.log('  node puppeteer-scheduler.js check       # Check availability once');
            console.log('  node puppeteer-scheduler.js earliest    # Find earliest available');
            console.log('  node puppeteer-scheduler.js status      # Show current status');
            console.log('  node puppeteer-scheduler.js set-last <date>   # Update last appointment');
            console.log('  node puppeteer-scheduler.js set-next <date>   # Set next appointment');
            console.log('');
